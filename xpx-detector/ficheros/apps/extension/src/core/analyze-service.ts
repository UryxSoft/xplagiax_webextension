...
export function startAnalyzeService(opts: AnalyzeServiceOptions): AnalyzeService {
  ...
  async function analyzeOnce(wire: WireInput): Promise<Verdict> {
    // La profundidad entra en la clave. El mismo texto analizado en Tier 1 y en
    // Tier 2 da veredictos distintos, y servir el superficial a quien pidió el
    // profundo sería devolver una abstención como si fuera una respuesta.
    const clave = wire.deep === true ? `${wire.hash}:deep` : wire.hash;

    const hit = cache.get(clave);
    if (hit !== undefined) return hit;

    const yaPedido = enVuelo.get(clave);
