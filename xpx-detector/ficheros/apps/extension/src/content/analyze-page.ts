...
export interface AnalyzePageOptions {
  ...
  /**
   * Análisis profundo: activa el detector de texto Tier 2.
   *
   * Descarga 253 MB la primera vez y tarda 0,5–2 s por bloque, así que solo
   * debe llegar aquí en `true` si el usuario lo pidió explícitamente.
   */
...
export async function analyzePage(opts: AnalyzePageOptions = {}): Promise<readonly Verdict[]> {
  ...
  const resultados = await Promise.allSettled(
    entradas.map((e) => cliente.request('analyze', { ...toWire(e), deep }, isVerdict)),
  );
  cliente.dispose();
  puerto.disconnect();
  ...
  renderBadge(doc, veredictos, { deep });
  return veredictos;
}
