...
export interface InferenceServerOptions {
   ...
   *
   * `deep` decide hasta qué tier se llega. Se pasa como argumento en lugar de
   * fijarlo al construir el servidor porque la profundidad la elige el usuario
   * en cada análisis, no el cableado.
   */
  readonly analyze: (input: NormalizedInput, deep: boolean) => Promise<Verdict>;
...
export function startInferenceServer(opts: InferenceServerOptions): InferenceServer {
  ...
  const onConnect = (port: PortLike): void => {
    ...
    const server = new RpcServer(portTransport(port)).on('infer', isWireInput, (wire) =>
      run(opts.analyze, fromWire(wire), wire.deep ?? false),
    );
 ...
 */
async function run(
  analyze: (input: NormalizedInput, deep: boolean) => Promise<Verdict>,
  input: NormalizedInput,
  deep: boolean,
): Promise<Verdict> {
  const verdict = await analyze(input, deep);
