...
export interface WireInput {
  ...
  /**
   * Pide análisis profundo (Tier 2).
   *
   * No forma parte de `NormalizedInput` y no debe: describe QUÉ se quiere hacer
   * con el contenido, no el contenido. Por eso viaja en el sobre y `fromWire`
   * lo ignora — el kernel nunca lo ve.
   *
   * Ausente o `false` significa Tier 1: sin él, el detector de texto (253 MB,
   * 0,5–2 s por bloque) ni siquiera se planifica. Es la garantía de ADR-005
   * hecha código, y el motivo de que este campo exista en lugar de subir el
   * `maxTier` por defecto.
   */
...
export function isWireInput(v: unknown): v is WireInput {
  ...
  const deep = v['deep'];
  if (deep !== undefined && typeof deep !== 'boolean') return false;
