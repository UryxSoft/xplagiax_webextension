...
describe('la caché distingue la profundidad', () => {
  /**
   * Servir el resultado superficial a quien pidió el profundo devolvería una
   * abstención como si fuera una respuesta. Es el peor fallo posible de una
   * caché: silencioso y con apariencia de corrección.
   */
  it('el mismo bloque en Tier 1 y en Tier 2 son dos análisis, no uno', async () => {
    const f = fakeChromium();
    const { host, llamadas } = hostFalso(async (i) => veredictoDe(i));
    const servicio = startAnalyzeService({ api: f.api, host });
    ...
    await cliente.request('analyze', toWire(entrada), isVerdict);
    await cliente.request('analyze', { ...toWire(entrada), deep: true }, isVerdict);

    expect(llamadas).toHaveLength(2);
    servicio.dispose();
  });

  it('pero repetir la misma profundidad sí sale de la caché', async () => {
    const f = fakeChromium();
    const { host, llamadas } = hostFalso(async (i) => veredictoDe(i));
    const servicio = startAnalyzeService({ api: f.api, host });
  ...
  it('el flag llega intacto al host de inferencia', async () => {
    const f = fakeChromium();
    const { host, llamadas } = hostFalso(async (i) => veredictoDe(i));
    const servicio = startAnalyzeService({ api: f.api, host });
    ...
    expect((llamadas[0] as { deep?: boolean } | undefined)?.deep).toBe(true);
    servicio.dispose();
  });
});
