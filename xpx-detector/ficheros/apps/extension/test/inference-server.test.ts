...
describe('profundidad del análisis', () => {
  /**
   * La garantía de ADR-005, comprobada en la frontera donde se decide: sin el
   * flag no se llega a Tier 2, y con él sí. Si esto se rompiera, cada bloque de
   * cada página dispararía la descarga de 253 MB.
   */
  function montarConProfundidad() {
    const f = fakeChromium();
    const recibido: boolean[] = [];
    const servidor = startInferenceServer({
      api: f.api,
      analyze: async (i, deep) => {
        recibido.push(deep);
        return pipelineCon(0).analyze(i);
      },
    });
    return { host: new ChromiumRuntimeHost({ api: f.api }), servidor, recibido };
  }

  it('sin el flag, el servidor recibe deep=false', async () => {
    const { host, servidor, recibido } = montarConProfundidad();
    await host.run('infer', toWire(entrada()), isVerdict);
    expect(recibido).toEqual([false]);
    servidor.dispose();
  });

  it('con el flag, el servidor recibe deep=true', async () => {
    const { host, servidor, recibido } = montarConProfundidad();
    await host.run('infer', { ...toWire(entrada()), deep: true }, isVerdict);
    expect(recibido).toEqual([true]);
    servidor.dispose();
  });

  it('un deep que no es booleano se rechaza en la frontera', async () => {
    const { host, servidor, recibido } = montarConProfundidad();
    await expect(
      host.run('infer', { ...toWire(entrada()), deep: 'sí' }, isVerdict),
    ).rejects.toMatchObject({ code: ErrorCode.BadRequest });
