 ...
 *
 * Sus `package.json` apuntan a `dist/`, que no existe hasta que se compilan; sin
 * esto el bundler falla con "Failed to resolve entry for package". Además evita
 * un orden de build implícito y garantiza una sola copia del kernel en el
 * artefacto. Es el mismo grafo que ya describen tsconfig y vitest.
 */
const workspaceAliases = {
  '@xpx/ipc': ruta('packages/ipc'),
  '@xpx/kernel': ruta('packages/kernel'),
  '@xpx/runtime': ruta('packages/runtime'),
  '@xpx/provenance': ruta('packages/detectors/provenance'),
  '@xpx/extinction-validator': ruta('packages/detectors/extinction-validator'),
  '@xpx/slop-detector': ruta('packages/detectors/slop'),
};

function ruta(paquete: string): string {
  return fileURLToPath(new URL(`../../${paquete}/src/index.ts`, import.meta.url));
}
