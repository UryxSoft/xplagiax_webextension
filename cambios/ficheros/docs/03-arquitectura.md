...
```
InferenceRuntime
├── OrtWebBackend        WebGPU → WASM+SIMD+threads → WASM simple   (pendiente)
└── WllamaBackend        GGUF, solo Tier 2, solo cuando el usuario opta
```

**Estado.** El camino de wllama está implementado (`WllamaTextClassifier`) y sostiene el detector
de texto Tier 2. `OrtWebBackend` sigue pendiente, así que hoy **no hay Tier 1**: el análisis de
texto es opt-in mientras no exista.

El paquete no importa `@wllama/wllama`. Declara la superficie que usa como interfaz
(`WllamaLike`) y la instancia real la inyecta la extensión, donde TypeScript comprueba la
compatibilidad estructural. Así el runtime compila y se prueba sin WASM ni navegador — mismo
criterio que `ExtensionApi` para las APIs de extensión.
