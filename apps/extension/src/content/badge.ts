...
export function renderBadge(doc: Document, verdicts: readonly Verdict[]): BadgeHandle {
  dispose(doc);
  ...
  shadow.append(estilos(doc), panel(doc, verdicts));
...
function panel(doc: Document, verdicts: readonly Verdict[]): HTMLElement {
  ...
  // limpio, cuando lo que ocurre es que todavía no hay quien lo examine.
  const nota = doc.createElement('p');
  nota.className = 'nota';
  nota.textContent = 'Sin detector de texto: el análisis de texto se abstiene siempre.';
  panel.append(nota);
